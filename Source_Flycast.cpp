...
int main()
{
    ...
    uintptr_t DreamcastRAMPointer = 0x0;
    uintptr_t DreamcastRAM0 = 0x0;
    if (GameSystems[SelectGameSystem] == "Sega Dreamcast (Flycast v1.3)") {
        if (Games[SelectGame] == "Tomb Raider 4") { // Tomb Raider 4.
            ...
            if (GameVersions[SelectVersion] == "783C5835E95DF837FEB87DB6367C1C9A") { // 783C5835E95DF837FEB87DB6367C1C9A
                ProcessName = L"flycast.exe";
                ModuleName = L"flycast.exe";
                DreamcastRAM0 = 0x13E1C000;
                NumberOfRoomsAddress = 0xDF3478;
                RoomPointer = 0xDF3480;
                FloorDataPointer = 0xDF3494;
            }
            else if (GameVersions[SelectVersion] == "C57D42CEB15BF4314C296653E2F5A55D") { // C57D42CEB15BF4314C296653E2F5A55D
                ProcessName = L"flycast.exe";
                ModuleName = L"flycast.exe";
                DreamcastRAM0 = 0x13E1D000;
                NumberOfRoomsAddress = 0xDF1EB4;
                RoomPointer = 0xDF1EBC;
                FloorDataPointer = 0xDF1ED0;
            }
            else if (GameVersions[SelectVersion] == "8C36CD2580BA32C87CFB80AD732FB38B") { // 8C36CD2580BA32C87CFB80AD732FB38B
                ProcessName = L"flycast.exe";
                ModuleName = L"flycast.exe";
                DreamcastRAM0 = 0x13E17000;
                NumberOfRoomsAddress = 0xDF3970;
                RoomPointer = 0xDF3978;
                FloorDataPointer = 0xDF398C;
            }
        }
        else if (Games[SelectGame] == "Tomb Raider 5") { // Tomb Raider 5.
            ...
            if (GameVersions[SelectVersion] == "B020DFE54C4EC0B7D32DD1C68F61E038") { // B020DFE54C4EC0B7D32DD1C68F61E038
                ProcessName = L"flycast.exe";
                ModuleName = L"flycast.exe";
                DreamcastRAM0 = 0x13D68000;
                NumberOfRoomsAddress = 0xC55A20;
                RoomPointer = 0xC55A28;
                FloorDataPointer = 0xC55A3C;
            }
            else if (GameVersions[SelectVersion] == "1D7B952AA49FF5474D994CE686539E82") { // 1D7B952AA49FF5474D994CE686539E82
                ProcessName = L"flycast.exe";
                ModuleName = L"flycast.exe";
                DreamcastRAM0 = 0x13D66000;
                NumberOfRoomsAddress = 0xC55A3C;
                RoomPointer = 0xC55A44;
                FloorDataPointer = 0xC55A58;
            }
        }
    }
    ...
    else if (GameSystems[SelectGameSystem] == "Sega Dreamcast (Flycast v1.3)") {
        ReadProcessMemory(hProcess, (LPCVOID)(ModuleBaseAddress + 0x2040AC0), &DreamcastRAMPointer, 8, NULL);
        DreamcastRAMPointer = DreamcastRAMPointer - ModuleBaseAddress + DreamcastRAM0;
        NumberOfRoomsAddress += DreamcastRAMPointer;
        NumberOfRoomsAddress;
        RoomPointer += DreamcastRAMPointer;
        FloorDataPointer += DreamcastRAMPointer;
    }
    ...
    if (GameSystems[SelectGameSystem] == "Sony Playstation (Bizhawk 2.8, core: OctoShock)") {
        ...
        RoomBaseAddress += ModuleBaseAddress + DreamcastRAMPointer;
        FloorDataBaseAddress += ModuleBaseAddress + DreamcastRAMPointer;
    }
    ...
    for (uint16_t RoomNumber = 0; RoomNumber < NumberOfRooms; RoomNumber++) {
        ...
        if (GameSystems[SelectGameSystem] == "Sony Playstation (Bizhawk 2.8, core: OctoShock)") {
            ...
            StartRoomData += ModuleBaseAddress + DreamcastRAMPointer;
            EndRoomData += ModuleBaseAddress + DreamcastRAMPointer;
        }
                ...
                do {
                    ...
                    case 0x15: // Function 0x15.
                        ...
                        if (GameSystems[SelectGameSystem] == "Personal Computer") {
                            ...
                            AddressesFile << "Address: 0x" << hex << uppercase << SectorFloorDataAddress << " (DreamcastRAM: 0x" << (SectorFloorDataAddress - ModuleBaseAddress - DreamcastRAMPointer) << ")" << dec << " Room: " << (unsigned short int)RoomNumber << " Sector: " << (unsigned short int)Sector << endl;
                        }
                        break;
                    }
                } while (EndData == 0x0);
    ...
    while (getline(GetInfoFromFile, GetLineString)) {
        ...
        if (GameSystems[SelectGameSystem] == "Personal Computer") {
            ...
            CheckFile << "Address: 0x" << hex << uppercase << AddressToCheck << " (DreamcastRAM: 0x" << (AddressToCheck - ModuleBaseAddress - DreamcastRAMPointer) << ")" << " (Room: " << dec << (unsigned short int)AddressRoom << ", Sector: " << (unsigned short int)AddressSector << ")" << endl;
        }
